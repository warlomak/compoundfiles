...
This fork adds additional compliance features:

Writing Compliance
------------------

The :class:`~compoundfiles.CompoundFileWriter` implements a fully compliant
Red-Black tree directory structure as required by the CFB specification.
This ensures that all written files meet the official standards and are
compatible with other CFB implementations.

Editing Compliance
------------------

The :class:`~compoundfiles.CompoundFileEditor` maintains compliance when
modifying existing files by preserving the original structure where possible
and applying appropriate validation when making changes.
