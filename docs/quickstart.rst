...
This fork adds the ability to create new compound files using the
:class:`CompoundFileWriter` class::

    >>> import compoundfiles
    >>> with compoundfiles.CompoundFileWriter('new_file.cfb') as writer:
    ...     # Create a storage
    ...     storage = writer.create_storage(writer.root, "MyStorage")
    ...
    ...     # Create a stream in the root
    ...     writer.create_stream(writer.root, "MyStream", b"Hello, Compound File!")
    ...
    ...     # Create a stream in the storage
    ...     writer.create_stream(storage, "NestedStream", b"This is inside a storage")

Editing Files
=============

The fork also provides editing capabilities through the
:class:`CompoundFileEditor` class::

    >>> import compoundfiles
    >>> with compoundfiles.CompoundFileEditor('existing_file.cfb') as editor:
    ...     # Add a new stream
    ...     editor.add_stream(editor.root, "NewStream", b"Additional data")
    ...
    ...     # Rename an existing entity
    ...     editor.rename("/OldName", "NewName")
    ...
    ...     # Delete an entity
    ...     editor.delete("/SomeStream")
    ...
    ...     # Save changes
    ...     editor.save()

For more complete examples, see the :doc:`examples` section.
