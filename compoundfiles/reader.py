...
#
# This file contains modified code for reading compound files.
...
class CompoundFileReader(object):
    ...
    def __init__(self, filename_or_obj):
        ...
        if len(self._mmap) < COMPOUND_HEADER.size:
            raise CompoundFileInvalidMagicError(
                '%s does not appear to be an OLE compound '
                'document (file too small: %d bytes, need %d)' %
                (filename_or_obj, len(self._mmap), COMPOUND_HEADER.size))
        ...
        # Calculate max sector index correctly
        total_sectors = (self._file_size - self._header_size + self._normal_sector_size - 1) // self._normal_sector_size
        self._max_sector = total_sectors - 1  # Index starts from 0
    ...
    def open(self, filename_or_entity):
        ...
        # Determine if this should be a mini stream based on size
        should_be_mini_stream = (filename_or_entity.size > 0 and
                                 filename_or_entity.size < self._mini_size_limit)

        # If it should be a mini stream but there's no mini FAT, raise an error
        if should_be_mini_stream and not self._mini_fat:
            raise CompoundFileNoMiniFatError(
                'attempt to open mini stream "%s" but no mini FAT available' % filename_or_entity.name)

        cls = (
                CompoundFileMiniStream
                if should_be_mini_stream and self._mini_fat else
                CompoundFileNormalStream)
        return cls(
    ...
    def get(self, path):
        """
        Get the entity at the specified path.

        :param path: Path to the entity (e.g., '/storage/stream')
        :return: CompoundFileEntity instance
        """
    ...
    def listdir(self, path="/"):
        """
        List the contents of a directory/storage at the specified path.

        :param path: Path to the directory/storage to list (default is root '/')
        :return: List of entity names in the directory
        """
        entity = self.get(path)
        if not entity.isdir:
            raise CompoundFileNotStorageError(
                '%s is not a storage' % path)

        return [item.name for item in entity]

    def exists(self, path):
        """
        Check if an entity exists at the specified path.

        :param path: Path to the entity to check
        :return: True if entity exists, False otherwise
        """
        try:
            self.get(path)
            return True
        except CompoundFileNotFoundError:
            return False
    ...
    def _load_normal_fat(self, master_sectors):
        ...
        # Extend the FAT array to the total number of sectors in the file
        # This ensures that the FAT array has entries for all sectors, not just those read from FAT sectors
        if len(self._normal_fat) <= self._max_sector:
            # Pad with FREE_SECTOR values
            padding_needed = (self._max_sector + 1) - len(self._normal_fat)
            self._normal_fat.extend([FREE_SECTOR] * padding_needed)
        else:
            # If FAT is larger than max sector, truncate it to max sector size + 1
            if len(self._normal_fat) > self._max_sector + 1:
                self._normal_fat = self._normal_fat[:self._max_sector + 1]
        ...
        for master_sector in master_sectors:
            if master_sector >= len(self._normal_fat):
                # This can happen if the master sector is beyond the actual file size
                continue
        ...
        for normal_sector in self._master_fat:
            if normal_sector >= len(self._normal_fat):
                # This can happen if the normal sector is beyond the actual file size
                continue
    ...
    def _load_mini_fat(self):
        ...
        if self._mini_first_sector == FREE_SECTOR:
            ...
            self._mini_first_sector = END_OF_CHAIN
        print(f"DEBUG READER: Loading MiniFAT - _mini_first_sector={self._mini_first_sector}, _mini_sector_count={self._mini_sector_count}")
        print(f"DEBUG READER: FAT array size = {len(self._normal_fat)}, Max valid index = {len(self._normal_fat)-1}")
        if self._mini_first_sector != END_OF_CHAIN:
            # Проверяем, что _mini_first_sector не является специальным значением
            if self._mini_first_sector >= len(self._normal_fat):
                print(f"ERROR: _mini_first_sector ({self._mini_first_sector}) >= FAT size ({len(self._normal_fat)})")
                print(f"This means the MiniFAT start sector is out of bounds!")
            # Проверяем, что _mini_first_sector не является специальным значением
            elif self._mini_first_sector in (NORMAL_FAT_SECTOR, MASTER_FAT_SECTOR, END_OF_CHAIN, FREE_SECTOR):
                print(f"WARNING: _mini_first_sector is a special value ({self._mini_first_sector}), skipping MiniFAT loading")
            # Проверяем, что сектор не помечен как занятый FAT
            elif self._mini_first_sector < len(self._normal_fat) and self._normal_fat[self._mini_first_sector] == NORMAL_FAT_SECTOR:
                print(f"WARNING: _mini_first_sector points to a sector marked as FAT, skipping MiniFAT loading")
            else:
